...
class LiveKF:
  def __init__(self, CP):
    self.frame = 0
    point_max_time = 10 * 60 # 10 minutes
    self.point_max_frame_diff = int(point_max_time / DT_MDL)
    ...
    self.abs_lat_jerk_check_pts = min(3, int(round((CP.steerActuatorDelay + 0.2) / DT_MDL)))
  ...
  def reset(self):
    ...
    self.lat_accel_ratio_bins_frames = [[deque(maxlen=self.bin_size) for _ in range(self.lat_accel_num_bins)] for _ in range(self.speed_num_bins)]
  ...
  def remove_old_points(self):
    for i in range(len(self.lat_accel_ratio_bins)):
      for j in range(len(self.lat_accel_ratio_bins[i])):
        while len(self.lat_accel_ratio_bins_frames[i][j]) > 0 and self.frame - self.lat_accel_ratio_bins_frames[i][j][0] > self.point_max_frame_diff:
          self.lat_accel_ratio_bins[i][j].popleft()
          self.lat_accel_ratio_bins_frames[i][j].popleft()
  ...
  def add_point(self, speed, desired_lateral_accel, actual_lateral_accel):
    ...
    if all([len(i) > 0 for i in [speed_bin, lat_accel_bin]]):
      ...
      self.lat_accel_ratio_bins_frames[speed_bin[0]][lat_accel_bin[0]].append(self.frame)
    ...
    for i in range(len(self.lat_accel_ratio_bins)):
      for j in range(len(self.lat_accel_ratio_bins[i])):
        for k in range(len(self.lat_accel_ratio_bins[i][j])):
          self.lat_accel_ratio_bins[i][j][k] += kf_ratio * (1.0 - self.lat_accel_ratio_bins[i][j][k]) * 0.01
